      ...
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white rounded-3xl shadow-xl shadow-sky-100 border border-sky-50 overflow-hidden">
          <div className="bg-gradient-to-r from-sky-500 to-teal-400 px-6 py-4">
            <p className="text-white font-bold text-sm">新規アカウント登録</p>
          </div>
          <form onSubmit={handleSubmit} className="px-6 py-6 space-y-4">
            <div>
              <label className="block text-xs font-bold text-sky-500 uppercase tracking-widest mb-1.5">名前</label>
              <input
                type="text" required value={name}
                onChange={(e) => setName(e.target.value)}
                placeholder="例：お父さん"
                className="w-full border-2 border-sky-100 rounded-xl px-4 py-2.5 text-sm font-medium text-gray-800 focus:outline-none focus:border-sky-400 transition-colors placeholder-gray-300"
              />
            </div>
            <div>
            ...
            <div>
              <label className="block text-xs font-bold text-sky-500 uppercase tracking-widest mb-1.5">パスワード（6文字以上）</label>
              <input
          ...
          <div className="px-6 pb-6 text-center">
            <p className="text-sm text-gray-400">
              すでにアカウントがある方は{" "}
              <Link href="/login" className="text-sky-600 font-bold hover:underline">ログイン</Link>
            </p>
          </div>
        </div>
      </div>
